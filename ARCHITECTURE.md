...
규칙을 지켰다고만 적으면 문서가 광고가 됩니다. 어긴 자리도 적어 둡니다.

`lib/` 의 카피 파일은 글만 든 파일이라 아무것도 의존하지 않는 것이 자연스럽지만,
두 곳에서 `components/` 를 가져다 씁니다.

```ts
import type { IconName } from "@/components/icons";   // lib/site-content.ts:7
import type { IconName } from "@/components/icons";   // lib/cook-content.ts:9
```

안쪽이 바깥쪽을 가리키는 유일한 자리입니다. **타입만** 빌려오는 것이라 빌드하면
사라지고 실제 의존은 남지 않습니다. 없는 아이콘 이름을 카피에 적으면 컴파일이
막히게 하려고 일부러 이렇게 두었습니다. 그 밖에는 `lib/` 의 어떤 파일도
`components/` 나 `app/` 을 import 하지 않습니다.
...
폴더 14개와 공통 컴포넌트 9개, 모두 47개입니다. 그중 **30개가 클라이언트
컴포넌트**(`"use client"`)입니다 — 음성·타이머·폼처럼 브라우저에서 살아 움직여야
하는 화면이 많아서입니다. `lib/` 에는 `"use client"` 가 한 줄도 없습니다.

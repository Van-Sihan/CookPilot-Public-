...
- 환경변수 없음 (`.env` 를 만들 필요가 없습니다)
- 상태 관리 라이브러리 없음 (`useState` 하나가 전부)
- 다국어 처리 없음 (한국어 고정)
- 로그인 단추와 홈셰프·키친 요금제 단추는 아직 `href="#"` 입니다

## 5. 클린 아키텍처가 실제로 적용된 곳

지금 규칙이 적용된 것은 **키 검증 한 갈래**뿐입니다. 나머지 화면은 아직 그릴
것만 있어 계층이랄 게 없습니다. 앞으로 기능이 붙을 때 갈 자리는 이렇습니다.
...
app/api/          라우트 핸들러. 입력을 받아 유스케이스를 부르기만 함
